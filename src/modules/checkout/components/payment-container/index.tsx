...
const PaymentContainer: React.FC<PaymentContainerProps> = ({
  ...
  return (
    <>
      <div className='flex flex-col gap-y-2 text-small-regular cursor-pointer py-4 border rounded-rounded px-8 mb-2 hover:shadow-borders-interactive-with-active'>Coming Soon! We are currently working on this feature. Please continue with manual payments for now.</div>
      <RadioGroup.Option
        key={paymentSession.id}
        value={paymentSession.provider_id}
        disabled={disabled}
        className={clx(
          "hidden flex-col gap-y-2 text-small-regular cursor-pointer py-4 border rounded-rounded px-8 mb-2 hover:shadow-borders-interactive-with-active",
          {
