...
export const listCustomerOrders = cache(async function (
  ...
  return medusaClient.customers
    .listOrders({ limit, offset, expand: "store" }, headers)
    .then(({ orders }) => {
      const filterdOrders = orders.filter((order) => order.store !== null)
      console.log("Total Orders:", filterdOrders.length);
    ...
    .list(
      {
        ...
        region_id: region.id,
        ...queryParams,
        // expand:"categories,collection,variants.calculated_price_type,variants.original_price,variants.calculated_price,,shipping_options,profiles"
      },
      { next: { tags: ["products"] } }
    )
    .then((res) => res)
